...
# SECURITY: pickle deserialization executes arbitrary code. These caches are
# local build artifacts regenerated by build_mapmaker_ops.py on this machine —
# only ever load caches you created yourself, never files from an untrusted
# source. (Plain-text pickle usage here was explicitly authorized by the
# maintainer; the previous obfuscated import was removed in review.)
...
def load_tods() -> list[np.ndarray]:
    data = np.load(
        os.path.join(DSA_DIR, "simulated_TODs_meerklass.npz"), allow_pickle=True
    )
