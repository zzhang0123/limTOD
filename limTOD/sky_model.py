...
if __name__ == "__main__":
    # Example usage
    freqs = np.linspace(900.0, 1200.0, 20)  # MHz

    # Vaguely foreground-like
    m = generate_gaussian_field(
        freqs=freqs, nside=64, amp=1.0, alpha=-3.0, beta=1.0, xi=0.1, fwhm=2.0
    )
    hp.mollview(m[0])
    hp.mollview(m[1])
    ...
    m = generate_gaussian_field(
        freqs=freqs, nside=64, amp=1.0, alpha=-1.0, beta=1.0, xi=0.01, fwhm=0.0
    )
    hp.mollview(m[0])
    hp.mollview(m[1])
