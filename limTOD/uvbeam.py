...
* UVBeam azimuth follows pyuvdata's convention (East = 0, North = pi/2,
  counterclockwise). limTOD's beam maps enter the pointing chain through
  ``zyz_of_pointing``, under which the beam-map meridian ``phi = pi/2``
...
def healpix_phi_to_uvbeam_az(phi_rad: np.ndarray) -> np.ndarray:
    ...
    ``az_uvbeam = pi/2 - phi_healpix``: through limTOD's pointing chain
    the beam-map meridian phi = pi/2 points toward local East (UVBeam
    az = 0) and phi = 0 toward local North (az = pi/2). Locked by the
